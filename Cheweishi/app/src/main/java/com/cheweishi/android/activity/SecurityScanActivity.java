...
public class SecurityScanActivity extends BaseActivity {
	...
	private void getDetectionData() {
		if (hasDevice()) {
			...
			Log.i("result", "uid = " + loginMessage.getUid() + "   mobile = "
					+ loginMessage.getMobile() + "   cid = "
					+ loginMessage.getCarManager().getId());
			ProgrosDialog.openDialog(this);
			httpBiz.httPostData(20000, API.CSH_CAR_DETECTION_URL, params,
					SecurityScanActivity.this);
		}
	}
	...
	private void seeDetectDetails() {
		if (!StringUtil.isEmpty(detectionInfos)) {
			...
			bundle.putParcelableArrayList("obdList",
					(ArrayList<? extends Parcelable>) detectionInfos);
			bundle.putParcelableArrayList("dtcList",
					(ArrayList<? extends Parcelable>) dtcInfos);
